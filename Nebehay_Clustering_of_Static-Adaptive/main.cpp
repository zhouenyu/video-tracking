...
int main()
{
	...
	int index = 0;
	int c;
	input_path = Read_ImgFilePath("config.txt");
	vector<string> files;
	readImageSequenceFiles(input_path, files);
	Mat im0 = imread(files[0]);
	rect = Read_objectbbox(im0);
	...
	if (challenge_flag)
	{
		//Read list of images
		vector<string> files;
		readImageSequenceFiles(Read_ImgFilePath("config.txt"), files);
